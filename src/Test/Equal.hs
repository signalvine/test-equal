...
std :: forall xss . (All2 Equal xss, SListI xss) => POP Comparator xss
std = hcpure (Proxy :: Proxy Equal) $ Comparator cmp

cmpFields
  :: forall a xss .
     ( Generic a, xss ~ Code a
     , All2 Equal xss, SListI xss
...
fieldNames ci =
  ...
  where
    ...
    genName :: State Int (FieldInfo xs)
    genName = do
      n <- get
      put $! n+1
...
nthConstructorName n conInfos =
  hcollapse $ hliftA2 (\constrInfo _ -> K $ conName constrInfo) conInfos n
